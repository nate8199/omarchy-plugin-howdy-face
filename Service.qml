...
Item {
  ...
  function startHowdy() {
    ...
    pendingHowdy = false
    howdyCancelled = false
    failureMessage = ""
    howdyAuthenticating = true
    runWake()
    howdyWatchdog.restart()
    howdyCompareProc.running = true
    if (!howdyCompareProc.running) {
      howdyAuthenticating = false
      howdyWatchdog.stop()
      handleHowdyFailure()
  ...
  function cancelHowdy() {
    if (!howdyAuthenticating && !howdyCompareProc.running && !pendingHowdy) return
  ...
  WlSessionLock {
    ...
    onSecureStateChanged: {
      ...
      if (secure) {
        ...
        if (root.pendingHowdy) root.startHowdy()
      }
    }
    ...
    WlSessionLockSurface {
      ...
      LockView {
        ...
        howdyConfigured: root.howdyConfigured
        authenticatingPassword: root.authenticatingPassword
        authenticatingHowdy: root.howdyAuthenticating
        ...
        onFaceUnlockRequested: root.startHowdy()
        onFaceUnlockCancelled: root.cancelHowdy()
      }
  ...
  Timer {
    ...
    repeat: false
    onTriggered: {
      if (!root.howdyAuthenticating) return
      root.logEvent("howdy-timeout")
      root.cancelHowdy()
      root.failureMessage = "Face unlock timed out"
    }
  }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (root.lockRequested && !root.authenticatingPassword && !root.howdyAuthenticating) root.runBlank()
    }
  }
  ...
  onHowdyAuthenticatingChanged: {
    if (!lockRequested) return
    if (howdyAuthenticating) idleBlankTimer.stop()
    else armBlankTimer()
  }
  ...
  IpcHandler {
    ...
    function cancelHowdy(): string {
      if (!root.howdyAuthenticating && !howdyCompareProc.running && !root.pendingHowdy) return "idle"
      root.cancelHowdy()
      return "ok"
    }
  }
